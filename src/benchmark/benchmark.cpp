...
#include <libwshash/util.h>
#ifdef OPENCL
#include <libwshash-cl/wshash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(wshash_h256_t const *hash, unsigned size)
{
...
extern "C" int main(void)
{
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		wshash_return_value hash;
		wshash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("wshash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		wshash_return_value hash;
		wshash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : wshash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			wshash_return_value hash;
			wshash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			wshash_return_value hash;
			#ifdef FULL
				wshash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				wshash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
	...
	unsigned read_size = WSHASH_ACCESSES * WSHASH_MIX_BYTES;
