...
*/
/** @file io.h
 * @author Lefteris Karapetsas <lefteris@wshdev.com>
 * @date 2015
 */
#pragma once
#include <stdlib.h>
...
// Reference: https://github.com/wiseplat/wiki/wiki/Wshash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see wshash_io_prepare
enum wshash_io_rc {
	WSHASH_IO_FAIL = 0,           ///< There has been an IO failure
	WSHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	WSHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	WSHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of wshash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * wshash_full_t
 */
#ifdef WSHASH_PRINT_CRITICAL_OUTPUT
#define WSHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("WSHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for wshash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the wshash
 ...
 *                           https://github.com/wiseplat/wiki/wiki/Wshash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum wshash_io_rc
 */
enum wshash_io_rc wshash_io_prepare(
	char const* dirname,
	wshash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* wshash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* wshash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool wshash_mkdir(char const* dirname);
 ...
 */
bool wshash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* wshash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/wiseplat/wiki/wiki/Wshash-DAG
 *
 ...
 */
bool wshash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool wshash_io_mutable_name(
	uint32_t revision,
	wshash_h256_t const* seed_hash,
	char* output
)
{
    uint64_t hash = *((uint64_t*)seed_hash);
#if LITTLE_ENDIAN == BYTE_ORDER
    hash = wshash_swap_u64(hash);
#endif
