...
mkcache_bytes(PyObject *self, PyObject *args) {
    ...
    wshash_light_t L = wshash_light_new(block_number);
...
calc_dataset_bytes(PyObject *self, PyObject *args) {
    ...
    if (cache_size % WSHASH_HASH_BYTES != 0) {
        char error_message[1024];
        sprintf(error_message, "The size of the cache must be a multiple of %i bytes (was %i)", WSHASH_HASH_BYTES, cache_size);
        PyErr_SetString(PyExc_ValueError, error_message);
        return 0;
    }

    wshash_params params;
    ...
    wshash_compute_full_data(mem, &params, &cache);
...
hashimoto_light(PyObject *self, PyObject *args) {
    ...
    struct wshash_light *s;
    s = calloc(sizeof(*s), 1);
    s->cache = cache_bytes;
    s->cache_size = cache_size;
    s->block_number = block_number;
    struct wshash_h256 *h;
    h = calloc(sizeof(*h), 1);
    for (int i = 0; i < 32; i++) h->b[i] = header[i];
    struct wshash_return_value out = wshash_light_compute(s, *h, nonce);
...
hashimoto_full(PyObject *self, PyObject *args) {
    ...
    wshash_return_value out;
    wshash_params params;
    params.full_size = (size_t) full_size;
    wshash_full(&out, (void *) full_bytes, &params, (wshash_h256_t *) header, nonce);
    return Py_BuildValue("{" PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT ", " PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT "}",
                         "mix digest", &out.mix_hash, 32,
                         "result", &out.result, 32);
}
...
mine(PyObject *self, PyObject *args) {
    ...
    do {
        wshash_full(&out, (void *) full_bytes, &params, (const wshash_h256_t *) header, nonce++);
        // TODO: disagrees with the spec https://github.com/wiseplat/wiki/wiki/Wshash#mining
    } while (!wshash_check_difficulty(&out.result, (const wshash_h256_t *) difficulty));
...
get_seedhash(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "k", &block_number))
        return 0;
    if (block_number >= WSHASH_EPOCH_LENGTH * 2048) {
        char error_message[1024];
        sprintf(error_message, "Block number must be less than %i (was %lu)", WSHASH_EPOCH_LENGTH * 2048, block_number);
...
static PyMethodDef PywshashMethods[] =
        {
...
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef PywshashModule = {
...
PyMODINIT_FUNC PyInit_pywshash(void) {
    PyObject *module =  PyModule_Create(&PywshashModule);
    // Following Spec: https://github.com/wiseplat/wiki/wiki/Wshash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) WSHASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) WSHASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) WSHASH_DATASET_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "CACHE_BYTES_INIT", (long) WSHASH_CACHE_BYTES_INIT);
    PyModule_AddIntConstant(module, "CACHE_BYTES_GROWTH", (long) WSHASH_CACHE_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "EPOCH_LENGTH", (long) WSHASH_EPOCH_LENGTH);
    PyModule_AddIntConstant(module, "MIX_BYTES", (long) WSHASH_MIX_BYTES);
    PyModule_AddIntConstant(module, "HASH_BYTES", (long) WSHASH_HASH_BYTES);
    PyModule_AddIntConstant(module, "DATASET_PARENTS", (long) WSHASH_DATASET_PARENTS);
    PyModule_AddIntConstant(module, "CACHE_ROUNDS", (long) WSHASH_CACHE_ROUNDS);
    PyModule_AddIntConstant(module, "ACCESSES", (long) WSHASH_ACCESSES);
    return module;
}
#else
PyMODINIT_FUNC
initpywshash(void) {
    PyObject *module = Py_InitModule("pywshash", PywshashMethods);
    // Following Spec: https://github.com/wiseplat/wiki/wiki/Wshash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) WSHASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) WSHASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) WSHASH_DATASET_BYTES_GROWTH);
