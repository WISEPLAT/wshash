...
def test_get_cache_size_not_None():
    for _ in range(100):
        ...
        out = pywshash.get_cache_size(block_num)
        assert out != None

def test_get_full_size_not_None():
    for _ in range(100):
        block_num = randint(0,12456789)
        out = pywshash.get_full_size(block_num)
        assert out != None

def test_get_cache_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pywshash.get_cache_size(block_num)
        out2 = pywshash.get_cache_size((block_num // pywshash.EPOCH_LENGTH) * pywshash.EPOCH_LENGTH)
        assert out1 == out2

def test_get_full_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pywshash.get_full_size(block_num)
        out2 = pywshash.get_full_size((block_num // pywshash.EPOCH_LENGTH) * pywshash.EPOCH_LENGTH)
        assert out1 == out2

# See light_and_full_client_checks in test.cpp
def test_mkcache_is_as_expected():
    actual = pywshash.mkcache_bytes(
        1024,
        "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~").encode('hex')
...
def test_calc_dataset_is_not_None():
    cache = pywshash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    assert pywshash.calc_dataset_bytes(1024 * 32, cache) != None
...
def test_light_and_full_agree():
    ...
    full_size = 1024 * 32
    header = "~~~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~"
    light_result = pywshash.hashimoto_light(full_size, cache, header, 0)
    dataset = pywshash.calc_dataset_bytes(full_size, cache)
    full_result = pywshash.hashimoto_full(dataset, header, 0)
...
def test_mining_basic():
    ...
    assert type(pywshash.mine(dataset,header,easy_difficulty)) == dict
    assert type(pywshash.mine(dataset,header,easy_difficulty)["nonce"]) == long
    assert type(pywshash.mine(dataset,header,easy_difficulty)["mix digest"]) == str
    assert type(pywshash.mine(dataset,header,easy_difficulty)["result"]) == str
...
def test_mining_doesnt_always_return_the_same_value():
    ...
    dataset = pywshash.calc_dataset_bytes(full_size, cache)
    # Check type of outputs
    assert pywshash.mine(dataset, header, easy_difficulty1)['nonce'] != pywshash.mine(dataset, header, easy_difficulty2)['nonce']

def test_get_seedhash():
    assert pywshash.get_seedhash(0).encode('hex') == '0' * 64
    import hashlib, sha3
    expected = pywshash.get_seedhash(0)
    #print "checking seed hashes:",
    for i in range(0, 30000*2048, 30000):
        #print i // 30000,
        assert pywshash.get_seedhash(i) == expected
