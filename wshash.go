...
/*
#include "src/libwshash/internal.h"

int wshashGoCallback_cgo(unsigned);
*/
import "C"
...
import (
	...
	"github.com/wiseplat/go-wiseplat/crypto"
...
func defaultDir() string {
	...
	if runtime.GOOS == "windows" {
		return filepath.Join(home, "AppData", "Wshash")
	}
	return filepath.Join(home, ".wshash")
}

// cache wraps an wshash_light_t with some metadata
...
func (cache *cache) generate() {
	cache.gen.Do(func() {
		...
		size := C.wshash_get_cachesize(C.uint64_t(cache.epoch * epochLength))
		if cache.test {
			size = cacheSizeForTesting
		}
		cache.ptr = C.wshash_light_new_internal(size, (*C.wshash_h256_t)(unsafe.Pointer(&seedHash[0])))
...
func freeCache(cache *cache) {
	C.wshash_light_delete(cache.ptr)
	cache.ptr = nil
}

func (cache *cache) compute(dagSize uint64, hash common.Hash, nonce uint64) (ok bool, mixDigest, result common.Hash) {
	ret := C.wshash_light_compute_internal(cache.ptr, C.uint64_t(dagSize), hashToH256(hash), C.uint64_t(nonce))
...
func (l *Light) VerifyShare(block Block, shareDiff *big.Int) (bool, bool, int64, common.Hash) {
	...
	// TODO: do wshash_quick_verify before getCache in order
	...
	/* Cannot happen if block header diff is validated prior to PoW, but can
	   ...
	   Wiseplat protocol consensus rules here which are not in scope of Wshash
	*/
	...
	dagSize := C.wshash_get_datasize(C.uint64_t(blockNum))
	if l.test {
...
func h256ToHash(in C.wshash_h256_t) common.Hash {
	return *(*common.Hash)(unsafe.Pointer(&in.b))
}

func hashToH256(in common.Hash) C.wshash_h256_t {
	return C.wshash_h256_t{b: *(*[32]C.uint8_t)(unsafe.Pointer(&in[0]))}
}
...
// dag wraps an wshash_full_t with some metadata
// and automatic memory management.
type dag struct {
	epoch uint64
	test  bool
...
func (d *dag) generate() {
	d.gen.Do(func() {
		var (
			...
			cacheSize = C.wshash_get_cachesize(blockNum)
			dagSize   = C.wshash_get_datasize(blockNum)
		)
		if d.test {
		...
		cache := C.wshash_light_new_internal(cacheSize, (*C.wshash_h256_t)(unsafe.Pointer(&seedHash[0])))
		defer C.wshash_light_delete(cache)
		// Generate the actual DAG.
		d.ptr = C.wshash_full_new_internal(
			C.CString(d.dir),
			hashToH256(seedHash),
			dagSize,
			cache,
			(C.wshash_callback_t)(unsafe.Pointer(C.wshashGoCallback_cgo)),
		)
		if d.ptr == nil {
			panic("wshash_full_new IO or memory error")
		}
...
//export wshashGoCallback
func wshashGoCallback(percent C.unsigned) C.int {
...
func (pow *Full) Search(block Block, stop <-chan struct{}, index int) (nonce uint64, mixDigest []byte) {
	...
	for {
		...
		default:
			...
			ret := C.wshash_full_compute(dag.ptr, hash, C.uint64_t(nonce))
...
// Wshash combines block verification with Light and
...
func New() *Wshash {
	return &Wshash{new(Light), &Full{turbo: true}}
}
...
func NewShared() *Wshash {
	return &Wshash{sharedLight, &Full{turbo: true}}
}
...
// regular-size cache.
func NewForTesting() (*Wshash, error) {
	dir, err := ioutil.TempDir("", "wshash-test")
	if err != nil {
		return nil, err
	}
	return &Wshash{&Light{test: true}, &Full{Dir: dir, test: true}}, nil
}
