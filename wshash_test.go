...
func TestWshashVerifyValid(t *testing.T) {
	wsh := New()
...
func TestWshashVerifyInvalid(t *testing.T) {
	wsh := New()
	if wsh.Verify(&invalidZeroDiffBlock) {
		t.Errorf("should not validate - we just ensure it does not panic on this block")
	}
}

func TestWshashConcurrentVerify(t *testing.T) {
	wsh, err := NewForTesting()
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(wsh.Full.Dir)

	block := &testBlock{difficulty: big.NewInt(10)}
	nonce, md := wsh.Search(block, nil, 0)
	block.nonce = nonce
...
func TestWshashConcurrentSearch(t *testing.T) {
	wsh, err := NewForTesting()
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < nsearch; i++ {
		go func() {
			nonce, md := wsh.Search(block, stop, 0)
			select {
...
func TestWshashSearchAcrossEpoch(t *testing.T) {
	wsh, err := NewForTesting()
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetSeedHash(t *testing.T) {
	...
	// From python:
	// > from pywshash import get_seedhash
	// > get_seedhash(30000)
